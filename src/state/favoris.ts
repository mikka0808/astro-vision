...
const state: FavorisState = {
  favoris: loadFavoris(),
  listes: loadListes()
};

const subscribers = new Set<FavorisSubscriber>();
...
function loadFavoris(): Set<string> {
  const stored = readStorage<string[]>(FAVORIS_KEY, { fallback: [] });
  if (!Array.isArray(stored)) {
    return new Set();
  }
  const ids = stored.filter((id): id is string => typeof id === 'string' && id.trim().length > 0);
  return new Set(ids);
}

function persistFavoris(ids: Set<string>) {
  writeStorage(FAVORIS_KEY, Array.from(ids));
}

function loadListes(): Liste[] {
  const stored = readStorage<unknown>(LISTES_KEY, { fallback: [] });
  if (!Array.isArray(stored)) {
    return [];
  }
  return stored
    .map((entry) => {
      ...
      const raw = entry as Partial<Liste>;
      const id = normalizeId(raw.id);
      const nom = normalizeId(raw.nom) || 'Liste sans titre';
      const cibleIds = Array.isArray(raw.cibleIds)
        ? raw.cibleIds.filter((value): value is string => typeof value === 'string' && value.trim().length > 0)
        : [];
      if (!id) {
        return null;
      }
    ...
    .filter((value): value is Liste => value !== null);
}

function persistListes(listes: Liste[]) {
...
function ensureListIndex(listId: string): number {
  return state.listes.findIndex((liste) => liste.id === listId);
}

function cloneListes(listes: Liste[]): Liste[] {
...
function sortListes(listes: Liste[]): Liste[] {
  return [...listes].sort((a, b) => a.nom.localeCompare(b.nom, 'fr', { sensitivity: 'base' }));
}

function setListes(listes: Liste[]) {
  state.listes = listes;
  persistListes(state.listes);
}

function setFavoris(ids: Set<string>) {
...
export const Favoris = {
  ...
  isFavori(id: string | null | undefined): boolean {
  ...
  remove(id: string | null | undefined): boolean {
    const normalized = normalizeId(id);
    if (!normalized) return false;
    if (!state.favoris.has(normalized)) {
      return false;
  ...
  renommerListe(listId: string, nom: string): Liste | null {
  ...
  supprimerListe(listId: string): boolean {
    const index = ensureListIndex(listId);
    if (index === -1) {
      return false;
    }
  ...
  addToListe(listId: string, cibleId: string): boolean {
  ...
  removeFromListe(listId: string, cibleId: string): boolean {
    const normalizedListe = normalizeId(listId);
    const normalizedCible = normalizeId(cibleId);
    if (!normalizedListe || !normalizedCible) {
      return false;
  ...
  subscribe(listener: FavorisSubscriber): () => void {
